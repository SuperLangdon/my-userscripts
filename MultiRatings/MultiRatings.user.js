...
// @name         豆瓣评分聚合
// @version      1.0
// @namespace    MultiRatingss
// @author       Langdon
// @description  在豆瓣电影页面显示 IMDb、Letterboxd 和 Rotten Tomatoes 的评分信息。
...
// @run-at       document-end
// @homepageURL  https://github.com/GeniusLangdon/my-userscripts
// @license      GPL-3.0
// @downloadURL
// @updateURL
// ==/UserScript==
...
(function() {
    ...
    const rtSectionHTML = `
        ...
        <div class="rating_self clearfix" style="margin-top: 8px; padding: 10px 0 0 0;">
            <div style="display: flex; flex-direction: column; gap: 6px;">
                <div style="display: flex; align-items: center;">
                    <div class="tomato-container" style="width: 36px; height: 36px; margin-right: 8px; display:flex; align-items:center; justify-content:center;"></div>
                    <div style="display:flex; flex-direction:column; gap:2px; line-height:1;">
                        <strong class="ll rating_num" style="margin-right: 0; font-size:22px; line-height:1;">--</strong>
                        <span class="rt-meta" style="color: #999; font-size: 12px;">影评人</span>
                    </div>
                </div>
                <div style="display: flex; align-items: center;">
                    <div class="popcorn-container" style="width: 36px; height: 36px; margin-right: 8px; display:flex; align-items:center; justify-content:center;"></div>
                    <div style="display:flex; flex-direction:column; gap:2px; line-height:1;">
                        <strong class="ll rating_num" style="margin-right: 0; font-size:22px; line-height:1;">--</strong>
                        <span class="rt-meta" style="color: #999; font-size: 12px;">观众</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    class MovieRatings {
        ...
        enhanceInfoLinks(imdbId) {
            const info = document.querySelector('#info');
            if (!info) return;

            const imdbUrl = `https://www.imdb.com/title/${imdbId}/`;
            this.makeImdbIdClickable(imdbId, imdbUrl);

            if (!this.infoLinks) {
                info.querySelector('.imdb-inline-links')?.remove();
                const imdbLink = info.querySelector('a.imdb-id-link');
                if (!imdbLink) return;

                const linksWrap = document.createElement('span');
                linksWrap.className = 'imdb-inline-links';
                linksWrap.innerHTML = `
                    <span style="color:#999;"> | </span>
                    <span class="ext-link-lb" style="color:#999;">Letterboxd</span>
                    <span> & </span>
                    <span class="ext-link-rt" style="color:#999;">Rotten Tomatoes</span>
                `;
                imdbLink.insertAdjacentElement('afterend', linksWrap);

                this.infoLinks = {
                    lb: linksWrap.querySelector('.ext-link-lb'),
                    rt: linksWrap.querySelector('.ext-link-rt')
                };
            }
        }

        makeImdbIdClickable(imdbId, imdbUrl) {
            const imdbSpan = Array.from(document.querySelectorAll('#info span.pl'))
                .find(span => span.textContent.includes('IMDb:'));
            if (!imdbSpan || imdbSpan.parentNode.querySelector('a.imdb-id-link')) return;

            const textNode = imdbSpan.nextSibling;
            const raw = textNode?.nodeType === Node.TEXT_NODE ? textNode.nodeValue : '';
            const ttId = raw?.match(/tt\d+/i)?.[0] || imdbId;

            const link = document.createElement('a');
            link.className = 'imdb-id-link';
            link.href = imdbUrl;
            link.target = '_blank';
            link.textContent = ttId;

            if (textNode?.nodeType === Node.TEXT_NODE) {
                textNode.nodeValue = ' ';
                imdbSpan.parentNode.insertBefore(link, textNode.nextSibling);
            } else {
                imdbSpan.insertAdjacentText('afterend', ' ');
                imdbSpan.parentNode.insertBefore(link, imdbSpan.nextSibling);
            }
        }

        setInfoLink(type, url) {
            if (!this.infoLinks?.[type]) return;
            const label = type === 'lb' ? 'Letterboxd' : 'Rotten Tomatoes';
            const a = document.createElement('a');
            a.href = url;
            a.target = '_blank';
            a.className = `ext-link-${type}`;
            a.textContent = label;
            this.infoLinks[type].replaceWith(a);
            this.infoLinks[type] = a;
        }

        setInfoLinkUnavailable(type) {
            if (!this.infoLinks?.[type]) return;
            const label = type === 'lb' ? 'Letterboxd' : 'Rotten Tomatoes';
            this.infoLinks[type].removeAttribute?.('href');
            this.infoLinks[type].textContent = label;
            this.infoLinks[type].style.color = '#999';
        }
        ...
        createRatingsSections() {
            ...
            // 创建 IMDb 评分区域
            this.imdbSection = this.createSection('imdb');

            // 创建烂番茄评分区域
            this.rtSection = this.createSection('rt', rtSectionHTML);
            this.lbSection = this.createSection('lb');

            // 插入到页面
            ratingsOnWeight.parentNode.insertBefore(this.lbSection, ratingsOnWeight.nextSibling);
            ratingsOnWeight.parentNode.insertBefore(this.imdbSection, this.lbSection.nextSibling);
            ratingsOnWeight.parentNode.insertBefore(this.rtSection, this.imdbSection.nextSibling);
            this.initImdbSection();
            this.initLetterboxdSection();
            this.initRatingsFold();
        }

        createSection(type, template) {
            const section = document.createElement('div');
            section.className = 'rating_wrap clearbox';
            section.style.cssText = (type === 'imdb' || type === 'lb')
        ...
        initImdbSection() {
            const doubanRatingSelf = document.querySelector(".rating_self");
            const doubanLogo = document.querySelector(".rating_logo");
            if (!doubanRatingSelf || !doubanLogo) return;

            const self = doubanRatingSelf.cloneNode(true);
            const logo = doubanLogo.cloneNode(true);
            logo.innerText = "IMDb 评分";

            const ratingNum = self.querySelector(".rating_num");
            const peopleLink = self.querySelector(".rating_people");
            const peopleSpan = peopleLink?.querySelector("span");
            const starElement = self.querySelector(".bigstar");

            if (ratingNum) ratingNum.innerText = "--";
            if (peopleSpan) peopleSpan.innerText = "--";
            if (peopleLink) peopleLink.removeAttribute("href");
            if (starElement) this.setBigstarClass(starElement, 0);

            this.imdbSection.innerHTML = "";
            this.imdbSection.appendChild(logo);
            this.imdbSection.appendChild(self);
        }

        initLetterboxdSection() {
            const doubanRatingSelf = document.querySelector(".rating_self");
            const doubanLogo = document.querySelector(".rating_logo");
            if (!doubanRatingSelf || !doubanLogo) return;
            ...
            this.lbSection.innerHTML = "";
            this.lbSection.appendChild(logo);
            this.lbSection.appendChild(self);
        }

        initRatingsFold() {
            this.moreExpanded = false;
            this.rtSection.style.display = 'none';
            this.imdbSection.style.display = 'none';

            const toggleWrap = document.createElement('div');
            toggleWrap.style.cssText = 'margin-top: 0px;';
            toggleWrap.innerHTML = `<a href="javascript:void(0);" class="ratings-fold-toggle">(展开更多)</a>`;
            this.lbSection.appendChild(toggleWrap);

            const toggleLink = toggleWrap.querySelector('.ratings-fold-toggle');
            toggleLink.addEventListener('click', (e) => {
                e.preventDefault();
                this.moreExpanded = !this.moreExpanded;
                this.rtSection.style.display = this.moreExpanded ? '' : 'none';
                this.imdbSection.style.display = this.moreExpanded ? '' : 'none';
                toggleLink.textContent = this.moreExpanded ? '(收起)' : '(展开更多)';
            });
        }
        ...
        async fetchImdbRatings(imdbId) {
            try {
                ...
                if (englishTitle) {
                    ...
                    await this.fetchLetterboxdRatings(imdbId, englishTitle);
                } else {
                    this.handleError('rt'); // 如果没有英文名，烂番茄直接报错
                    this.handleError('lb');
                }
            } catch (error) {
                this.handleError('imdb');
                this.handleError('rt'); // IMDb失败，烂番茄也无法进行
                this.handleError('lb');
            }
        }
        ...
        extractImdbRating(doc) {
            // 获取评分
            const scoreItem = doc.querySelector("[data-testid=hero-rating-bar__aggregate-rating__score] span");
            const score = scoreItem ? scoreItem.innerText : 'N/A';

            // 获取评分人数
            let voteCount = '0';
            try {
                const str = doc.querySelector("#__NEXT_DATA__").textContent;
                const json = JSON.parse(str);
                voteCount = json.props.pageProps.aboveTheFoldData.ratingsSummary.voteCount;
            } catch (e) {
                console.error("解析 IMDb 评分人数失败:", e);
            }

            return { score, count: voteCount };
        }
        ...
        async fetchRottenTomatoesRatings(title) {
            try {
                ...
                if (!movieLink) throw new Error('未找到匹配的 Rotten Tomatoes 页面');

                const moviePath = movieLink.getAttribute('href');
        ...
        async fetchLetterboxdRatings(imdbId, title) {
            try {
                // 优先通过 IMDb ID 定位，避免搜索页结果不稳定
                if (imdbId) {
                    const imdbUrl = `https://letterboxd.com/imdb/${imdbId}/`;
                    const imdbResponse = await this.makeRequest(imdbUrl);
                    if (!/No-one has added/i.test(imdbResponse.responseText)) {
                        const finalUrl = imdbResponse.finalUrl || imdbUrl;
                        this.processLetterboxdPage(imdbResponse, finalUrl);
                        return;
                    }
                }
                ...
                const searchResponse = await this.makeRequest(searchUrl);
                const searchDoc = new DOMParser().parseFromString(searchResponse.responseText, 'text/html');
                const movieLink = searchDoc.querySelector('ul.results li.film-detail-content h2 a')
                    || searchDoc.querySelector('ul.results li.poster-container a')
                    || searchDoc.querySelector('a[href^="/film/"]');
                if (!movieLink) throw new Error('未找到匹配的 Letterboxd 页面');

                const moviePath = movieLink.getAttribute('href');
                const movieUrl = moviePath.startsWith('http') ? moviePath : `https://letterboxd.com${moviePath}`;
                const movieResponse = await this.makeRequest(movieUrl);
                this.processLetterboxdPage(movieResponse, movieUrl);
            } catch (error) {
                this.handleError('lb');
            }
        }
        ...
        processRottenTomatoesPage(response, movieUrl) {
            ...
            this.setInfoLink('rt', movieUrl);
        }

        processLetterboxdPage(response, movieUrl) {
            const doc = new DOMParser().parseFromString(response.responseText, 'text/html');
            const scoreData = this.extractLetterboxdScore(doc, response.responseText);
            this.updateLetterboxdUI(scoreData, movieUrl);
            this.setInfoLink('lb', movieUrl);
        }
        ...
        extractLetterboxdScore(doc, rawHtml = '') {
            // 1) 优先解析 ld+json
                ...
                try {
                    ...
                    for (const item of items) {
                        const data = item['@type'] === 'Movie'
                            ? item
                            : (item.itemListElement?.find(x => x.item?.['@type'] === 'Movie')?.item);
                        const aggregateRating = data?.aggregateRating;
                        if (aggregateRating?.ratingValue) {
                            return {
                                score: aggregateRating.ratingValue,
                                count: aggregateRating.ratingCount || '0'
                            };
                        }
                    }
                } catch (e) {}
            ...
            // 2) 回退：解析常见 meta/script 文本
            const html = rawHtml || '';
            const scoreFromMeta = html.match(/name=["']twitter:data2["'][^>]*content=["']\s*([\d.]+)\s*out of 5/i)?.[1];
            const scoreFromJson = html.match(/"ratingValue"\s*:\s*"?([\d.]+)"?/i)?.[1];
            const countFromJson = html.match(/"ratingCount"\s*:\s*"?([\d.,]+[KMBkmb]?)"?/i)?.[1];
            const watchedMetaText = html.match(/name=["']twitter:data1["'][^>]*content=["']([^"']+)["']/i)?.[1] || '';
            const watchedCount = watchedMetaText.match(/([\d.,]+[KMBkmb]?)/)?.[1];

            const score = scoreFromMeta || scoreFromJson;
            const count = countFromJson || watchedCount;
            if (score || count) {
                return { score: score || 'N/A', count: count || '0' };
            }
        ...
        updateImdbUI(ratingData, imdbId) {
            const doubanRatingSelf = document.querySelector(".rating_self");
            const doubanLogo = document.querySelector(".rating_logo");
            if (!doubanRatingSelf || !doubanLogo) {
                this.handleError('imdb');
                return;
            }
            ...
            // 修改UI内容
            logo.innerText = "IMDb 评分";
            self.querySelector(".rating_num").innerText = ratingData.score;
            self.querySelector(".rating_people span").innerText = ratingData.count.toLocaleString(); // 添加千位分隔符
            self.querySelector(".rating_people").href = `https://www.imdb.com/title/${imdbId}/ratings`;

            // 修改星星
            const starElement = self.querySelector(".bigstar");
            if (starElement) {
                const starClass = starElement.classList.item(1); // bigstar45
                if (starClass) {
                    const newStarClass = "bigstar" + Math.round(parseFloat(ratingData.score) * 5);
                    starElement.classList.replace(starClass, newStarClass);
                }
            }
        }
        ...
        updateRottenTomatoesUI(scoreData, movieUrl) {
            ...
            const [criticsCount, audienceCount] = this.rtSection.querySelectorAll('.rt-meta');
        ...
        updateLetterboxdUI(scoreData, movieUrl) {
            if (!scoreData) {
                this.handleError('lb');
                return;
            }

            const ratingElement = this.lbSection.querySelector('.rating_num');
            const countElement = this.lbSection.querySelector('.rating_people span');
            const peopleLink = this.lbSection.querySelector('.rating_people');
            const starElement = this.lbSection.querySelector('.bigstar');

            const score = Number(scoreData.score);
            const count = this.normalizeCount(scoreData.count);
            ratingElement.textContent = Number.isFinite(score) ? score.toFixed(1) : 'N/A';
            countElement.textContent = Number.isFinite(count) ? count.toLocaleString() : '暂无';
            peopleLink.href = movieUrl;
            ratingElement.style.cursor = 'pointer';
            peopleLink.style.cursor = 'pointer';
            ratingElement.onclick = () => window.open(movieUrl, '_blank');
            peopleLink.onclick = () => window.open(movieUrl, '_blank');

            if (starElement && Number.isFinite(score)) {
                // 豆瓣星级 class 仅支持 5 的步进：00/05/10.../50
                const newStarValue = Math.max(0, Math.min(50, Math.round(score * 2) * 5));
                this.setBigstarClass(starElement, newStarValue);
            } else if (starElement) {
                this.setBigstarClass(starElement, 0);
            }
        }
        ...
        updateCriticsScore(criticsScore, ratingElement, countElement) {
            ...
            ratingElement.style.fontSize = '22px';
            ratingElement.style.lineHeight = '1';
        ...
        setBigstarClass(starElement, value) {
            if (!starElement) return;
            const numericValue = Number(value);
            const safeValue = Number.isFinite(numericValue) ? numericValue : 0;
            const normalizedValue = Math.max(0, Math.min(50, Math.round(safeValue / 5) * 5));
            const newStarClass = `bigstar${String(normalizedValue).padStart(2, '0')}`;
            const oldStarClass = Array.from(starElement.classList).find(cls => /^bigstar\d{2}$/.test(cls));

            if (oldStarClass) {
                if (oldStarClass !== newStarClass) {
                    starElement.classList.replace(oldStarClass, newStarClass);
                }
            } else {
                starElement.classList.add(newStarClass);
            }
        }
        ...
        handleError(type) {
            if (type === 'imdb') {
                const errorHTML = `
                    <div class="rating_logo ll">IMDb 评分</div>
                    <div class="rating_self clearfix">
                        <strong class="ll rating_num" style="font-size:14px; color:#999;">获取失败</strong>
                    </div>`;
                this.imdbSection.innerHTML = errorHTML;
            } else if (type === 'rt') {
                const [criticsRating, audienceRating] = this.rtSection.querySelectorAll('.rating_num');
                const [criticsCount, audienceCount] = this.rtSection.querySelectorAll('.rt-meta');
                ...
                criticsCount.textContent = '影评人';
                audienceCount.textContent = '观众';
                this.rtSection.querySelector('.tomato-container').innerHTML = TOMATO_ICONS.empty;
                this.rtSection.querySelector('.popcorn-container').innerHTML = POPCORN_ICONS.empty;
                this.setIconStyle(this.rtSection.querySelector('.tomato-container'));
                this.setIconStyle(this.rtSection.querySelector('.popcorn-container'));
                this.setInfoLinkUnavailable('rt');
            } else if (type === 'lb') {
                const ratingElement = this.lbSection.querySelector('.rating_num');
                const countElement = this.lbSection.querySelector('.rating_people span');
                const peopleLink = this.lbSection.querySelector('.rating_people');
                ...
                ratingElement.textContent = 'N/A';
                countElement.textContent = '暂无';
                peopleLink.removeAttribute('href');
                if (starElement) {
                    this.setBigstarClass(starElement, 0);
                }
                this.setInfoLinkUnavailable('lb');
            }
        }
